...
//     $message=escape($link, $_POST['message']);
    $message=preg_replace('/<(.*)s(.*)c(.*)r(.*)i(.*)p(.*)t/', '', escape($link, $_POST['message']));
    $query="insert into message_2(content,time) values('$message',now())";
...
<div id="xsss_main">
    ...
    <div id="show_message">
        ...
        <?php echo $html;?>
        <?php 
        $query="select * from message_2";
        $result=execute($link, $query);
        while($data=mysqli_fetch_assoc($result)){
            echo "<p class='con'>{$data['content']}</p><a href='../../inc/notice/message2_del.php?id={$data['id']}'>删除</a>";       
        }
        //输出的时候没有进行安全处理，导致XSS
        ?>
    </div>
...
<div id="right_bar">
	<p class="tips">tips:<br />很明显,这里过滤了&lt;script&gt;标签,过滤是一种常用的手段,这只是一个简单的例子,在实际的场景中,你可能需要打开脑洞,搞很多奇形怪状的姿势来绕过限制!</p>
</div>
<div id="bottom">
